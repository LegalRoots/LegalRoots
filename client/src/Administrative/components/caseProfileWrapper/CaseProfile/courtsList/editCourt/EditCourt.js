...
const EditCourt = ({
  ...
  useEffect(() => {
    if (employeesData) {
      ...
      tmp = court.permissions
        .filter((per) => {
          return (
            per.role === "user" &&
            per.userId !== pickedCase.plaintiff &&
            per.userId !== pickedCase.defendant
          );
        })
        .map((per) => per.userId);
      setSelectedWitnesses(tmp);
    }
  }, [court, employeesData, pickedCase]);

  const formData = {
    date: {
      value: `${new Date(court.time).getFullYear()}-${String(
        new Date(court.time).getMonth() + 1
      ).padStart(2, "0")}-${String(new Date(court.time).getDay()).padStart(
        2,
        "0"
      )}`,
      isValid: false,
    },
    timeH: { value: new Date(court.time).getHours(), isValid: false },
    timeM: { value: new Date(court.time).getMinutes(), isValid: false },
    initiator: { value: court.initiator, isValid: false },
    description: { value: court.description, isValid: false },
  ...
  const submitHandler = async (event) => {
    ...
    try {
      const response = await fetch(
        `${REACT_APP_API_BASE_URL}/admin/court/${court._id}`,
        {
          method: "PUT",
          headers: {
